...
public class Lexical {
    ...
    private ReserveTable reserveWords = new ReserveTable(50); //a few more than # 
//reserves
    private ReserveTable mnemonics = new ReserveTable(50); //a few more than # 
//reserves
    ...
    public Lexical(String filename, SymbolTable symbols, boolean echoOn) {
        ...
        try {
            ...
            bufferedreader = new BufferedReader(filereader);  //creates a buffering
...
//global char
    ...
    private token getIdent() {
    	// init identifier token
    	token result = new token();
    	result.code = IDENT_ID;
    	result.mnemonic = mnemonics.LookupCode(result.code);
    	// init the lexeme we're reading
        String lexeme = currCh + "";
        currCh = GetNextChar();
        // read all characters and digits into the lexeme
        while(isLetter(currCh) || isDigit(currCh) || currCh == '_')
        {
        	lexeme += currCh;
        	currCh = GetNextChar();
        }
        // when we read something different, we end the lexeme, add it to our token, and return
        result.lexeme = lexeme;
        int reserveCode = reserveWords.LookupName(lexeme);
        if(reserveCode != -1)
        {
    ...
    private token getNumber() {
        ...
        {
        	result.code = 99;
        	// assume error until exponent complete
        	lexeme += currCh;
        	currCh = GetNextChar();
        	// check for a + or -
        	if(currCh == '+' || currCh == '-')
        	...
        	while(isDigit(currCh))
        	{
       			// we now have confirmation that the exponent is complete
       			result.code = FLOAT_ID;
       			lexeme += currCh;
       			currCh = GetNextChar();
        	}
    ...
    private token getString() {
        ...
        // add a double quote to the end of the string
        lexeme += '"';
        result.lexeme = lexeme;
        // check if we have a warning for unterminated string
        if(currCh == '\n')
        {
        	System.out.println("Unterminated string found.");
        }
        // move past " or eol
       	currCh = GetNextChar();
        return result;
    ...
    private token getOneTwoChar() {
    	// init token and our peeking character
        token result = new token();
        char nextCh = PeekNextChar();
        // init lexeme
        String lexeme = "";
        // check for two character tokens (or 1 character prefixes)
        if(currCh == ':')
        {
        	if(nextCh == '=')
        	{
        ...
        // otherwise our lexeme is just our starting character
        else
        {
        	lexeme = currCh + "";
        }
        // search for these lexemes using the index to calculate the code
    	String[] acceptedLexemes = {"/","*","+","-","(",")",";",":=",">","<",">=","<=","=","<>",",","[","]",":","."};
    	int index = 0;
    	// assume unknown until we identify it
    	int code = 99;
    	// search through each accepted lexeme
    	for(index = 0; index < acceptedLexemes.length; index++)
    	{
    		// check if we found the matching lexeme
    		if(acceptedLexemes[index].equals(lexeme))
    		{
    			// calculate the token code based on the index in the array
    			code = index + 30;
    		}
    	}
    	// return the result
    	result.lexeme = lexeme;
    	result.code = code;
    	result.mnemonic = mnemonics.LookupCode(code);
    	currCh = GetNextChar();
        return result;
    };
        ...
        switch (result.code) {
            case IDENT_ID:
                if (len > 20) {
                    ...
                    System.out.println("Identifier length > 20, truncated " + 
result.lexeme
                            + " to " + lexemetrunc);
                }
               	if(integerOK(result.lexeme))
               	{
               		ival = (int)Integer.valueOf(result.lexeme);
               		saveSymbols.AddSymbol(result.lexeme, 'c', ival);
               	}
               	else
               	{
            ...
            case INTEGER_ID:
                if (len > 6) {
                    ...
                    System.out.println("Integer length > 6, truncated " + 
result.lexeme
                            + " to " + lexemetrunc);
                    ival = 0;
                    if (integerOK(lexemetrunc)) {
            ...
            case FLOAT_ID:
                if (len > 12) {
                    ...
                    System.out.println("Float length > 12, truncated " + 
result.lexeme
                            + " to " + lexemetrunc);
                    dval = 0;
                    if (doubleOK(lexemetrunc)) {
...
String.format("%04d", result.code) + " | \t" + result.lexeme);
    ...
    public int codeFor(String token)
    {
    	return mnemonics.LookupName(token);
    }
    
    public String reserveFor(String name)
    {
    	return reserveWords.LookupCode(mnemonics.LookupName(name));
    }
}
